...
本 Skill 是已注册 TeamFlow Agent 处理看板任务的统一入口。TeamFlow 自身的安装、配置、daemon 与诊断属于 `teamflow-setup`。
...
- **交接后立即结束 turn。** 路由、提交、评审或转交成功后不要轮询卡片等待其他 Agent。后续需要当前职责处理时，TeamFlow daemon 会创建新的通知 turn。
