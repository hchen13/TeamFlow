...
def main() -> int:
    ...
    listen_events_parser = subparsers.add_parser("listen-lark-events", help="Stream daemon-routed Bitable events for diagnostics.")
    ...
    daemon_parser.add_argument(
        "action",
        choices=("run", "start", "status", "stop", "enable", "disable", "sync"),
        help="Start/stop the global daemon, enable/disable this workspace, or synchronize it.",
    )
...
def cmd_verify_lark_user_identity(args: argparse.Namespace) -> int:
    ...
    if status.get("tokenStatus") == "valid":
        try:
            ...
            pass
    result = verify_lark_user_identity(args.workspace, status=status, profile=profile)
    if daemon_status()["running"] and workspace_enabled(args.workspace):
        sync_daemon_workspace(args.workspace)
    print_json(result)
    return 0
...
def cmd_initialize_lark_board(args: argparse.Namespace) -> int:
    print_json(initialize_lark_board(
        args.workspace,
        task_prefix=args.task_prefix,
        emit=lambda message: print(f"teamflow: {message}", file=sys.stderr, flush=True),
    ))
    return 0
...
def cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if args.action == "enable":
        print_json(enable_daemon_workspace(args.workspace, identity_id=args.identity_id))
        return 0
